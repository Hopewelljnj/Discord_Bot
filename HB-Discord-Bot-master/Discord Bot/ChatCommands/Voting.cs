...
namespace Discord_Bot
{
    ...
    public class Vote
    {
        ...
        public Vote Create(params VoteObject<T>[] objects)
        {
        ...
        public void CreateVote(uint id, Discord.User u)
        {
            ...
            uint preId = UserHasVoted(u);

            if (preId!= 1000)
            {
                Entries[preId].voteAmount -= 1; // Remove previous vote
                Entries[preId].UsersVoted.Remove(u.Id);
                return;
            }
        ...
        uint UserHasVoted(Discord.User u)
        {
